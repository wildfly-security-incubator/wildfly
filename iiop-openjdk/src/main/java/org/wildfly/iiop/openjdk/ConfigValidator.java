...
public class ConfigValidator {
    ...
    public static void validateConfig(final OperationContext context, final ModelNode resourceModel) throws OperationFailedException {
        ...
        validateIORTransportConfig(context, resourceModel, supportSSL, serverRequiresSsl, clientRequiresSsl);
    ...
    private static void validateIORTransportConfig(final OperationContext context, final ModelNode resourceModel, final boolean sslConfigured,
        ...
        validateTrustInClient(context, resourceModel, sslConfigured, clientRequiresSsl);
    ...
    private static void validateTrustInClient(final OperationContext context, final ModelNode resourceModel, final boolean sslConfigured,
                                              final boolean clientRequiresSsl) throws OperationFailedException {
        ...
        if (establishTrustInClientNode.isDefined()) {
            ...
            if (!sslConfigured && !establishTrustInClient.equalsIgnoreCase(Constants.NONE)) {
                throw IIOPLogger.ROOT_LOGGER.inconsistentUnsupportedTransportConfig(Constants.IOR_TRANSPORT_TRUST_IN_CLIENT);
            }
            if (clientRequiresSsl && !establishTrustInClient.equalsIgnoreCase(Constants.IOR_REQUIRED)) {
                throw IIOPLogger.ROOT_LOGGER.inconsistentRequiredTransportConfig(Constants.SECURITY_CLIENT_REQUIRES_SSL, Constants.IOR_TRANSPORT_TRUST_IN_CLIENT);
            }
            if (establishTrustInClient.equalsIgnoreCase(Constants.IOR_REQUIRED) && !clientRequiresSsl) {
                throw IIOPLogger.ROOT_LOGGER.inconsistentNotRequiredTransportConfig(Constants.SECURITY_CLIENT_REQUIRES_SSL, Constants.IOR_TRANSPORT_TRUST_IN_CLIENT);
            }
        }
    }
